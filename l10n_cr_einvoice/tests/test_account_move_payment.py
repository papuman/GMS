...
class TestAccountMovePayment(EInvoiceTestCase):
    ...
    def test_efectivo_without_transaction_id_success(self):
        ...
        invoice = self._create_test_invoice(payment_method=self.payment_method_efectivo)
    ...
    def test_sinpe_movil_without_transaction_id_error(self):
        ...
        invoice = self._create_test_invoice(payment_method=self.payment_method_sinpe)
    ...
    def test_tarjeta_without_transaction_id_success(self):
        ...
        invoice = self._create_test_invoice(payment_method=self.payment_method_tarjeta)
    ...
    def test_requires_einvoice_compute(self):
        ...
        invoice_cr = self._create_test_invoice()
        ...
        self.assertFalse(invoice_us.l10n_cr_requires_einvoice,
                        "Non-CR invoice should not require e-invoice")

    def test_validation_only_for_cr_invoices(self):
        """Test that payment method validation only applies to CR invoices."""
        # Create US company and partner
        ...
        # Create US invoice without payment method
        invoice_us = self.env['account.move'].create({
            'move_type': 'out_invoice',
            'partner_id': partner_us.id,
        ...
        # Should post without payment method validation
        invoice_us.action_post()
        self.assertEqual(invoice_us.state, 'posted', "US invoice should post without CR payment method")
