...
class D101IncomeTaxReport(models.Model):
    ...
    def action_submit_to_hacienda(self):
        ...
        if result.get('success'):
            ...
            self.message_post(body=_('D-101 submitted to Hacienda: %s') % self.submission_key)
        else:
            self.state = 'error'
    ...
    def action_check_status(self):
        ...
        if result.get('state') == 'accepted':
            ...
            self.message_post(body=_('D-101 accepted by Hacienda'))
            self.period_id.state = 'accepted'

        elif result.get('state') == 'rejected':
            self.state = 'rejected'
            self.hacienda_message = result.get('message', 'Rejected')
            self.message_post(body=_('D-101 rejected: %s') % self.hacienda_message)
