...
- ❌ Version 4.3 **OBSOLETE** (except credit/debit notes for pre-Sept invoices)
- ⚠️ **No grace period** - immediate enforcement
...
- **Base fine:** ₡8,300,000+ (~$14,800 USD)
...
- ✅ Must continue when switching providers
- ✅ Maximum: 9,999,999,999 (then can reset to 1)
...
**Receiver (Receptor):**
- Full name
- Tax ID (cédula física or jurídica)
...
**What is CABYS?**
- Catálogo de Bienes y Servicios (Catalog of Goods and Services)
...
**Requirement:** Address must use official Province → Canton → District codes
...
**Example - San José, Escazú, San Antonio:**
```
...
**Legal Basis:** Código de Normas y Procedimientos Tributarios (Tax Code)

**Base Fine:**
- ₡8,300,000 (~$14,800 USD) for non-compliance
- Up to ₡460,000 (~$821 USD) **per invoice** for violations
...
|------|---------------|--------------|-------|
| **01** | Electronic Invoice | Factura Electrónica | Standard invoice for goods/services |
| **02** | Debit Note | Nota de Débito | Additional charges to existing invoice |
| **03** | Credit Note | Nota de Crédito | Refunds, cancellations, corrections |
| **04** | Electronic Ticket | Tiquete Electrónico | Simplified invoice (POS, cash sales) |
| **05** | Purchase Acceptance | Confirmación de Aceptación de Comprobante | Purchase order acceptance |
| **06** | Partial Purchase Acceptance | Aceptación Parcial del Comprobante | Partial acceptance |
| **07** | Purchase Rejection | Rechazo del Comprobante | Purchase order rejection |
| **08** | Electronic Purchase Invoice | Factura Electrónica de Compra | Purchase from supplier |
| **09** | Export Electronic Invoice | Factura Electrónica de Exportación | Export sales |
...
**Replaces:**
- Legacy ATV (Administración Tributaria Virtual)
- TRAVI
- DeclaraWeb
- EDDI-7
...
- Classification: Emisor-Receptor Electrónico
...
**Fix:**
- Implement cascading dropdowns (Province → Canton → District)
...
**💻 For Implementation:**
- [Phase 3: Hacienda API](../../../PHASE3_API_INTEGRATION.md) - Technical integration
...
**KEY TAKEAWAY:** Version 4.4 compliance is MANDATORY with no grace period. Non-compliance results in severe penalties (₡8.3M+ base fine). GMS MUST implement all required fields, consecutive numbering, digital signatures, and CABYS codes to avoid legal liability.
